...
- **MCP-on-remote — Steps 2 + 3 + 4 + 7 + 8 (LANDED, `v0.6.2`).** `codemux-remote serve` headless daemon + `codemux-remote mcp` stdio bridge + 11-tool headless catalog + bearer-token auth + systemd user unit + auto-register of MCP in agent configs on every serve start + background `hosts_upgrade` poller. The desktop's "Push workspace to host" flow is now zero-touch — the user clicks once and an agent on the host has a working MCP control plane. Self-contained `src-tauri/src/remote/` module ships without first extracting `codemux_core` from the Tauri crate. Plan: `docs/plans/mcp-on-remote.md`. Feature: `docs/features/remote-hosts.md`.
