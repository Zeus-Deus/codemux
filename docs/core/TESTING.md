...
- `scripts/e2e/*.sh` — live Docker-backed harnesses run manually from the repo root:
  ...
  - `monitoring-status-e2e.sh` — the provider-agnostic "Monitoring" status over the real control surface: boots the debug binary in headless `serve` mode against a throwaway `HOME`/XDG root and drives `create_workspace` → `monitor_start` → snapshot/`monitor_status` agreement → `monitor_stop` → idempotency. No display, GUI, provider, or network needed; requires a debug build (built automatically if missing), python3, git, and a free TCP port (`CMX_MON_E2E_PORT`). See `docs/features/monitoring-status.md`.
