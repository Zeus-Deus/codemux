...
- **Environment-sensitive test: `agent_browser::tests::resolve_binary_finds_native_binary_from_project_root`** — fails locally on any machine where a system-installed `agent-browser` exists in `$PATH` (e.g. `/usr/bin/agent-browser` from the AUR `codemux-bin` package). The test asserts `resolve_binary()` returns a `node_modules/agent-browser/bin/agent-browser-linux-x64` style path, but the resolver's PATH-search branch wins first when a system binary is present. CI is unaffected because the runner has no system install. Workaround: skip the test locally with `cargo test --lib -- --skip resolve_binary_finds_native_binary_from_project_root`. Real fix is to gate the assertion on absence of `which agent-browser`, or to make the test prepend a fake `node_modules` `agent-browser-linux-x64` to a clean `PATH`
