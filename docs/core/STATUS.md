...
(`v0.17.0` and `v0.16.0`). The version in
`package.json`/`Cargo.toml`/`tauri.conf.json` is **`v0.17.0`** (released 2026-08-05);
everything listed under "Unreleased after `v0.17.0`" below landed after that tag and has
...
- **Settings → Usage: real token/cost accounting for all three chat providers.** A new append-only ledger records every increment of billable work — `ProviderRuntimeEvent::UsageRecorded` emitted by the Claude, Codex and OpenCode translators, intercepted in `forward_event`, written to the new `agent_usage_ledger` table (schema **v11**). It is a *parallel* path to the composer's `ContextUsageUpdated` meter, not a reuse of it: that event is a lossy, no-op-suppressed snapshot that every adapter deliberately strips subagent activity from, which makes it useless as an accounting record. The ledger is a delta, never suppressed, and counts subagent tokens with a `subagent` flag instead of dropping them. The four token fields are **non-overlapping**, which each adapter has to normalize into: Codex carves `cachedInputTokens` **and** `cacheWriteInputTokens` out of `inputTokens` (pinned arithmetically against the protocol decode fixture), OpenCode keeps its cache tiers as *siblings* of input, and Claude's block is already disjoint but repeats the same `message.id` with a **cumulative** usage block across content-block groups — so recording is per-id positive-delta, unit-tested and reconciled against the turn-end `result.modelUsage` map. Costs are list price frozen at record time (`agent_provider/pricing.rs`, substring-matched by model id, unknown model → no cost); OpenCode additionally keeps its provider catalogue's real per-token prices, which `flatten_model` used to discard into a single `is_free` boolean. The table carries **no foreign key** on purpose — deleting a chat from the history dropdown must not rewrite last month's spend. The page (design variant 2a) is an overview card (four hero stats, Cost⇄Tokens toggle, hover readout, hand-rolled stacked bars — no chart library was added) plus expandable provider lanes with per-model rows, and a CSV export. Billing kind is **detected** where the provider says so and hardcoded otherwise (see the quota item below); the design's terminal-CLI-log toggle is deliberately **not** implemented, since Codemux has no data behind it. See `docs/features/usage-dashboard.md`.

- **Settings → Usage: plan quota meters and real billing-mode detection.** The data was already crossing the wire and being dropped as untyped `RuntimeWarning`s. A second typed event, `ProviderRuntimeEvent::PlanUsageUpdated`, now carries per-provider quota *level* state (windows, plan label, detected auth mode) — snapshot semantics like `ContextUsageUpdated`, never accumulated, never persisted (a quota reading's meaning decays in minutes, so a replayed snapshot would be confidently wrong; it repopulates as sessions run). Claude decodes the SDK's `rate_limit_event`, which the CLI emits **only for claude.ai subscription logins** — so its presence alone is a reliable plan-covered signal. Codex gained the `account/rateLimits/updated` notification (previously falling through to `Unknown`), a best-effort `account/rateLimits/read` at session init so meters populate without waiting on a push, and now uses the `account/read` block it was already fetching and discarding for auth mode + plan label. The scaling trap is handled at the adapter boundary: Claude's `utilization` is a **0..1 fraction** while Codex's `usedPercent` is already a percent, and both `resetsAt` values are epoch **seconds**. `billing_kind_for` now takes a detected mode that wins in both directions — Claude on a raw API key bills as metered — falling back to the old hardcoded map, keeping one override point for the future setting. The lanes render up to two bars (5h + overall weekly) with status-token tone thresholds and a local-time reset note; Claude's per-model weekly windows move into the note rather than becoming extra bars. OpenCode has no quota API and its lane is unchanged. See `docs/features/usage-dashboard.md`.
