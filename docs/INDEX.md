...
- Subagent view (cross-provider Subagents orchestration card + read-only drill-in — Claude / Codex / OpenCode; PR #125, shipped `v0.11.0`): `docs/features/agent-chat.md` (§ Subagent view (cross-provider)); locked plan + implementation record at `docs/plans/subagent-view.md`
...
- Thread Scope (first-send scope controls moved below the composer on BOTH the draft surface and the pane's new-thread empty state — location/checkout/branch `ThreadScopeRow`, deferred + auto-named worktree creation on first send, Home draft hides the preset bar; on `main`, unreleased after `v0.11.0`): `docs/features/agent-chat.md` (§ Thread Scope (first-send scope controls))
- Workflow orchestration (Claude-only — `Workflow` tool_use tap → in-thread `WorkflowRunCard` approval/progress/summary + conditional Orchestration right-panel tab with per-phase agent drill-in + `/workflow` slash command; on `main`, unreleased after `v0.11.0`): `docs/features/workflow-orchestration.md`
