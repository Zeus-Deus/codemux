...
- Chat code blocks (Shiki syntax highlighting themed from the live terminal ANSI palette — `shiki-chat-theme.ts` + the module-level `use-chat-code-plugin.ts` store, `chat.code_wrap` Appearance toggle; PR #216 shipped in `v0.15.6`, file-aware parity follow-up PR #236 unreleased after `v0.15.6`): `docs/features/agent-chat.md` (§ Code blocks in chat), `docs/reference/DESIGN-SYSTEM.md`
- Rich chat links (generic external `http(s)` link favicons, safe-link behavior preserved, globe fallback for streaming/non-public/failed hosts; PR #237, unreleased after `v0.15.6`): `docs/features/agent-chat.md` (§ Rich external links in chat)
- Local screenshot links in chat (absolute PNG/JPEG/GIF/WebP Markdown links or image syntax → inline preview card + click-to-expand lightbox; asset-protocol desktop path with a bounded web-remote/dev fallback): `docs/features/agent-chat.md` (§ Local screenshot links in chat)
...
- Agent Tasks panel (read-only provider plan — the durable `TasksUpdated` snapshot from Claude `TodoWrite`/`Task*`, Codex `turn/plan/updated`, OpenCode `todo.updated`; thread-focused via `useActiveChatTasks`, conditional composer chip + right-panel tab; PR #229, unreleased after `v0.15.6`): `docs/features/agent-chat.md` (§ Agent Tasks panel)
- Context-window meter (composer donut-ring + popover showing live context occupancy `% · used/max`, total-processed, auto-compaction note — `ContextUsageUpdated` event from all three providers, persisted for hydrate-replay; PR #231, unreleased after `v0.15.6`): `docs/features/agent-chat.md` (§ Context-window meter (composer))
...
- Command palette (Ctrl+K unified switcher — Workspaces / Projects / Commands in one scored fuzzy search, `>` command mode, `/` path mode, status-first resting order, commands routed through the shared keybind `dispatch()`; from the `Command Palette.dc.html` design handoff; PR #230, unreleased after `v0.15.6`): `docs/features/command-palette.md`
...
- GUI responsiveness (the 2026-07-31 responsiveness audit — optimistic workspace selection, cursor-based chat hydration + lazy tool-result bodies, transcript memoization over the LegendList window, deferred terminal teardown, a change-gated background scheduler, and revisioned `app-state-delta` domains; **all seven phases landed, unreleased after `v0.15.6`** — what is left is the measurement matrix against the plan's budgets, plus the React Compiler trial): plan + remaining work at `docs/plans/gui-responsiveness.md`; current behavior in `docs/features/agent-chat.md` (§ Hydration by cursor), `docs/features/sidebar.md` (§ Selection and update cost), `docs/features/terminal.md`, `docs/features/observability.md` (§ Interaction Trace Harness)
