...
- Local screenshot links in chat (absolute PNG/JPEG/GIF/WebP Markdown links or image syntax → inline preview card + click-to-expand lightbox; asset-protocol desktop path with a bounded web-remote/dev fallback via `agent_chat_read_local_image`, same 25 MB ceiling as chat attachments; PR #239, unreleased after `v0.16.0`): `docs/features/agent-chat.md` (§ Local screenshot links in chat)
- New-turn scroll contract (a composer submission is a navigation intent, not a data update — `sendAnchor` + nonce-resolved anchor row feeding LegendList's `anchoredEndSpace`, the three states `following-end`/`anchoring-turn`/`free-scrolling`, follow released only by real scroll gestures, geometry in the unit-tested `send-scroll-state.ts`; the old `scrollToBottomSignal` scalar is gone; PR #247, unreleased after `v0.16.0`. Refined by PR #249, also unreleased: the anchor **survives the turn settling** — no more settle-time viewport yank — via the pane-owned `positionedNonceRef` remount guard + `onSizeChanged` advance, the send positioning became an animated glide with a `scrollend`/750ms settle handshake and a `prefers-reduced-motion` fallback, and the pill/follow edge signal widened to `isNearEnd`): `docs/features/agent-chat.md` (§ Transcript scroller → "The new-turn scroll contract")
...
- Workflow orchestration (Claude-only — `Workflow` tool_use tap → in-thread `WorkflowRunCard` approval/progress/summary + conditional Orchestration right-panel tab with per-phase agent drill-in + `/workflow` slash command; PR #137, shipped `v0.12.0`): `docs/features/workflow-orchestration.md`. **Not to be confused with** the legacy standalone orchestration workspace runtime, which was deleted outright by PR #246 (unreleased after `v0.16.0`) — its Rust module, frontend, commands, virtual-display/sandbox prototype, and feature doc are gone; schema v10 drops the stale history table and a retired-workspace-kind sink strips old sessions at restore. There is no current doc for it by design; the historical record survives only in `docs/archive/release-notes-v0.6-v0.13.md`.
...
- Cross-provider skill discovery + explicit invocation (implemented — provider-native catalogs as discovery truth, exact collision-safe selection, no global discovery toggle, and no automatic foreign-skill federation): `docs/archive/cross-provider-skill-support.md`; current behavior in `docs/features/agent-chat.md` (§ Cross-provider skill system)
