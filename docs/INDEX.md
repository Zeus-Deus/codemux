...
- Usage dashboard (Settings → Usage — token/cost accounting per provider, model and session from the local `agent_usage_ledger`, including subagent activity; a `UsageRecorded` ledger path parallel to (and deliberately separate from) the composer's `ContextUsageUpdated` meter, plus `PlanUsageUpdated` plan-quota meters and detected billing mode for Claude/Codex; Agent Chat GUI only): `docs/features/usage-dashboard.md`
...
- Design system (color tokens, theming layers, `.theme-warm`/density scales, the no-hardcoded-colors rule — shadcn stone base `b1HYEHloH`; **shipped in `v0.17.0`**: the `--selection-background`/`--selection-foreground` token pair driving one root-scoped `::selection` rule, with the renderer opt-out list that keeps CodeMirror syntax colors on selected code (PR #240), and the composer focus treatment that signals focus by surface tint + elevation rather than a border change (PR #242)): `docs/reference/DESIGN-SYSTEM.md`
