...
- Application theming (five built-ins, synced custom themes, Theme Studio generation/import/export, no-flash boot, and shared DOM/editor/chat/terminal palette). **Unreleased** (Theming design handoff, turn 3): theme *picking* moved into the command palette — ⌘K → "theme", highlight to live-preview the whole app, Enter to keep, Esc to revert — and Settings → Appearance's swatch grid became a single row stating the applied theme (every other control on that page is unchanged). Turn 4 turned the Theme Studio into a centered modal over Settings — Generate / Import tabs, an in-modal live preview instead of applying to the real app, role editing behind a link, parse-on-paste, and VS Code Marketplace search by name (`src-tauri/src/vscode_marketplace.rs`): `docs/features/theming.md`, `docs/features/command-palette.md` (§ The theme picker); completed implementation record at `docs/archive/theming.md`
- Tool permissions (Settings → Permissions — read/remove the agent's `allow`/`deny`/`ask` rules across the user/project/local `settings.json` scopes; Agent Chat GUI only): `docs/features/permissions.md`
- Agent Chat (the default interface — chat pane, providers, sidecar, attachments; default-on with a Settings → Interface opt-out back to CLI): `docs/features/agent-chat.md`
- Chat code blocks (Shiki syntax highlighting themed from the live terminal ANSI palette — `shiki-chat-theme.ts` + the module-level `use-chat-code-plugin.ts` store, `chat.code_wrap` Appearance toggle; PR #216 shipped in `v0.15.6`, file-aware parity follow-up PR #236 shipped in `v0.16.0`): `docs/features/agent-chat.md` (§ Code blocks in chat), `docs/reference/DESIGN-SYSTEM.md`
- Rich chat links (generic external `http(s)` link favicons, safe-link behavior preserved, globe fallback for streaming/non-public/failed hosts; PR #237, shipped in `v0.16.0`): `docs/features/agent-chat.md` (§ Rich external links in chat)
- Local screenshot links in chat (absolute PNG/JPEG/GIF/WebP Markdown links or image syntax → inline preview card + click-to-expand lightbox; asset-protocol desktop path with a bounded web-remote/dev fallback via `agent_chat_read_local_image`, same 25 MB ceiling as chat attachments; PR #239, shipped in `v0.17.0`): `docs/features/agent-chat.md` (§ Local screenshot links in chat)
- New-turn scroll contract (a composer submission is a navigation intent, not a data update — `sendAnchor` + nonce-resolved anchor row feeding LegendList's `anchoredEndSpace`, the three states `following-end`/`anchoring-turn`/`free-scrolling`, follow released only by real scroll gestures, geometry in the unit-tested `send-scroll-state.ts`; the old `scrollToBottomSignal` scalar is gone; PR #247, shipped in `v0.17.0`. Refined by PR #249, unreleased after `v0.17.0`: the anchor **survives the turn settling** — no more settle-time viewport yank — via the pane-owned `positionedNonceRef` remount guard + `onSizeChanged` advance, the send positioning became an animated glide with a `scrollend`/750ms settle handshake and a `prefers-reduced-motion` fallback, and the pill/follow edge signal widened to `isNearEnd`): `docs/features/agent-chat.md` (§ Transcript scroller → "The new-turn scroll contract")
- Stale provider requests (an approval or `AskUserQuestion` answered after the provider session restarted — `ProviderError::RequestNotPending`, the persisted `RequestResponseFailed` event, the OpenCode survives-restart opt-out, and the resolved-state reducer guard; PR #220, shipped in `v0.15.6`): `docs/features/agent-chat.md` (§ Pending requests are not conversation history)
- Agent Tasks panel (read-only provider plan — the durable `TasksUpdated` snapshot from Claude `TodoWrite`/`Task*`, Codex `turn/plan/updated`, OpenCode `todo.updated`; thread-focused via `useActiveChatTasks`, conditional composer chip + right-panel tab; PR #229, shipped in `v0.16.0`): `docs/features/agent-chat.md` (§ Agent Tasks panel)
- Context-window meter (composer donut-ring + popover showing live context occupancy `% · used/max`, total-processed, auto-compaction note — `ContextUsageUpdated` event from all three providers, persisted for hydrate-replay; PR #231, shipped in `v0.16.0`): `docs/features/agent-chat.md` (§ Context-window meter (composer))
...
- Monitoring status (the calm fifth `PaneStatus` for background watch loops — Claude watch-loop task classification settling a thread to `Monitoring` instead of pinning it at `Working`, the provider-agnostic `codemux monitor start|stop|status` CLI + socket commands, the docked `MonitoringBar` with its Stop button, and the steady cyan `--status-monitoring` token; unreleased after `v0.17.0`): `docs/features/monitoring-status.md`
- Subagent view (cross-provider Subagents orchestration card + read-only drill-in — Claude / Codex / OpenCode; PR #125, shipped `v0.11.0`; the per-thread `SubagentActivityBar` that replaced the pane-header/titlebar "N subagents running" pills landed in PR #143, shipped `v0.13.0`. **Unreleased** (Canvas-3 design handoff, change 3 of 3): the bordered card, tinted rows and `Enter ›` buttons are gone — a spawn group is a 1.5px state rail with inline-expanding hover rows and a one-line settled collapse, and the activity bar became a 32px strip welded inside the composer's top edge with a 1px accent sweep): `docs/features/agent-chat.md` (§ Subagent view (cross-provider), § Composer running strip); locked plan + implementation record at `docs/archive/subagent-view.md`
...
- Workflow orchestration (Claude-only — `Workflow` tool_use tap → in-thread `WorkflowRunCard` approval/progress/summary + conditional Orchestration right-panel tab with per-phase agent drill-in + `/workflow` slash command; PR #137, shipped `v0.12.0`): `docs/features/workflow-orchestration.md`. **Not to be confused with** the legacy standalone orchestration workspace runtime, which was deleted outright by PR #246 (shipped in `v0.17.0`) — its Rust module, frontend, commands, virtual-display/sandbox prototype, and feature doc are gone; schema v10 drops the stale history table and a retired-workspace-kind sink strips old sessions at restore. There is no current doc for it by design; the historical record survives only in `docs/archive/release-notes-v0.6-v0.13.md`.
...
- Skills sync (server-side, Step 10; six user scan roots incl. `~/.agents/skills/` and `~/.config/opencode/skills/`, kept in lockstep with `skills::paths::enumerate_scan_paths` by build-failing guard tests — PR #217, shipped in `v0.15.6`): `docs/features/skills-sync.md`; plan + per-stage history at `docs/archive/step-10-skills-sync.md`; research at `docs/research/step-10-skills-sync-research.md`; operator UI smoke at `docs/archive/step-10-ui-smoke-checklist.md`
- Cross-provider skill discovery + explicit invocation (implemented — provider-native catalogs as discovery truth, exact collision-safe selection, no global discovery toggle, and no automatic foreign-skill federation; PR #243, shipped in `v0.17.0`): `docs/archive/cross-provider-skill-support.md`; current behavior in `docs/features/agent-chat.md` (§ Cross-provider skill system)
...
- Historical Step 13 Agent Chat beta gate (retired when Agent Chat became default-on in PR #232): `docs/archive/step-13-beta-toggle-research.md`; operator UI smoke at `docs/archive/step-13-ui-smoke-checklist.md`; current interface control in `docs/features/settings.md`
...
- Local persistence layer (the storage substrate: the desktop `codemux.db` SQLite store and its tables, the separate `codemux-remote` daemon DB, the replay-idempotent migration ledger and why `SCHEMA_VERSION` gates nothing, what lives in `layout.json`/`observability.json` instead, and the rules for adding a column or a persisted field): `docs/features/persistence-layer.md`
- Session persistence (scrollback save/restore + agent session resume): `docs/features/session-persistence.md`
...
- Command palette (Ctrl+K unified switcher — Workspaces / Projects / Themes / Commands in one scored fuzzy search, `>` command mode, `/` path mode, status-first resting order, commands routed through the shared keybind `dispatch()`; from the `Command Palette.dc.html` design handoff; PR #230, shipped in `v0.16.0`. **Unreleased**: the search-only **Themes** group from the `Theming.dc.html` handoff — live full-app preview on the highlighted row, Enter to keep, Esc to revert, two studio rows at its foot): `docs/features/command-palette.md`
...
- Non-git projects (plain-folder mode — `is_git` snapshot flag, opt-in "Initialize Git" bare-`git init` affordance in the Context Row cluster / Changes panel, worktree controls hidden in Thread Scope, no add-path git gate; PRs #147/#148, shipped `v0.13.1`): `docs/features/workspace-creation.md` (§ Non-Git Projects)
...
- Project identity (first-class deterministic `project_uid` + `main`/`worktree` kind + boot sweep, shipped in `v0.7.4`): current behavior in `docs/features/workspaces-sync.md` and `docs/features/workspaces-overview.md`; completed implementation record at `docs/archive/project-identity.md`
...
- Left sidebar (two-state: expanded **flat workspace inbox** ↔ 52px rail. The inbox — shipped `v0.15.0`, from the `Sidebar Inbox.dc.html` design handoff — replaced the nested project tree (and the PR #182 living-sidebar strip/LIVE-grouping) with search + a project-filter dropdown (with active-workspace counts) + one card per active workspace + a persisted "Settled" section with settle/auto-settle/un-settle lifecycle, 288px default width, and Settings → Appearance → Sidebar "Show git stats" + "Auto-settle idle work" toggles; the rail was rebuilt as a per-workspace avatar strip with individual status dots and select-without-expand (the old project-avatar rail + hover flyout was deleted). **Unreleased**: the configurable working indicator (glyph picker + color swatches) was deleted and replaced by the shared monochrome **agent orb** — see `docs/features/agent-chat.md` § "Agent activity orb". Shipped `v0.15.5`: backend-owned activity stamps, a "Wrapping up" tier, snooze, a bulk-action safety net (PR #206), an oldest-blocked-first pinned needs-you strip with newest-first ordering (PR #207), and a shared workspace hover details card on every surface (PR #208). Shipped `v0.15.6`: PR badges restored on settled rows plus PR-completion settlement, palette indexing of settled workspaces by project path, row-scoped keyboard activation, and the two-column meta line (PRs #219/#221). Shipped in `v0.16.0`: the backend branch/repository PR association feeding settlement rather than exact SHA equality (PR #234), plus background-card attention receding (PR #227). Unreleased after `v0.17.0`: durable workspace Pin/Unpin with a pinned-first card block, lifecycle-preserving visibility override, parking safeguards, and collapsed-rail pin visibility (PR #248); settled rows dimmed until hover/focus, using the active card's `receded` exclusions verbatim (PR #256); the active card's PR chip matched to the settled-row badge in paint, click-through and keyboard scoping (PR #253); a hover card that keeps up with the pointer via a 150ms first-hover delay plus a shared instant-open group phase (PR #255); and badge-only side-branch PR association that never drives auto-settle): `docs/features/sidebar.md`
- Retired workspace context bar (the full-width bottom strip was removed; workspace detail lives in the sidebar/hover card and Agent Chat Context Row, while a live detached browser gets a compact active-terminal header control; PR #245, shipped in `v0.17.0`): `docs/features/workspace-context-bar.md`
...
- GUI chrome (Agent Chat GUI on — the default: the four chrome rows collapse to a single `h-10` title bar with in-titlebar tabs + `+` agent launcher + inline chat favorite + rehomed right-panel/Run controls; legacy chrome byte-identical when the flag is off; PR #135, shipped `v0.12.0` — the in-titlebar "N subagents running" pill was later removed by PR #143 in favor of the docked `SubagentActivityBar`. **Shipped in `v0.17.0`** (PR #245 + the direct commits that landed with it): the title bar became **frameless floating islands** — no full-width surface or divider, adjacent surfaces reach the physical top edge, and the two 32px islands raise an opaque surface only when a transcript has scrolled under them *and* an island intersects the centered reading column (`src/lib/titlebar-content-under.ts`); terminal pane chrome went transparent and is deliberately **flag-independent**; `useTitlebarOverlay()` is now the single gate for every top-edge collision clearance; both drag layers are desktop-only; and the right-panel toggle uses `PanelRight` in *both* chrome modes (PR #244 — the one deliberate exception to flag-off byte-identity)): `docs/features/gui-chrome.md`
- Notifications: `docs/features/notifications.md`
- Auto-update: `docs/features/auto-update.md`
- Windows cross-platform work: current gaps in `docs/plans/windows-support.md`; original port audit and smoke-test history in `docs/archive/windows-support-2026-04-audit.md`
...
- One-command remote bootstrap (`scripts/install.sh` + `codemux login`/`connect` + the `codemux.service` user unit — the two-command fresh-VPS story; client side landed, hosted deploys gated): `docs/plans/one-command-remote-bootstrap.md`; current CLI behavior in `docs/features/web-remote-access.md` (§ One-command bootstrap) + `docs/features/auth.md` (§ CLI Sign-In)
...
- GUI responsiveness (the 2026-07-31 responsiveness audit — optimistic workspace selection, cursor-based chat hydration + lazy tool-result bodies, transcript memoization over the LegendList window, deferred terminal teardown, a change-gated background scheduler, and revisioned `app-state-delta` domains; **all seven phases shipped in `v0.16.0`** — what is left is the measurement matrix against the plan's budgets, plus the React Compiler trial): plan + remaining work at `docs/plans/gui-responsiveness.md`; current behavior in `docs/features/agent-chat.md` (§ Hydration by cursor), `docs/features/sidebar.md` (§ Selection and update cost), `docs/features/terminal.md`, `docs/features/observability.md` (§ Interaction Trace Harness)
- Repo boundaries: `docs/reference/ARCHITECTURE.md`
- Design system (semantic role tokens, runtime application-theme bridge, density/radius scales, and the no-hardcoded-colors rule): `docs/reference/DESIGN-SYSTEM.md`
...
- Historical release notes, moved out of `docs/core/STATUS.md` (which keeps only the unreleased-on-`main` section plus the two most recent releases): `docs/archive/release-notes-v0.14-v0.15.md` (`v0.14.0`–`v0.15.6`) and `docs/archive/release-notes-v0.6-v0.13.md` (`v0.6.1`–`v0.13.2`)
- Other archived design notes, kept for historical reasoning only — all three cite source paths that predate later refactors: branch-picker redesign (superseded by `docs/features/workspace-creation.md`) at `docs/archive/branch-picker-redesign.md`; the GitHub-issues gap analysis (superseded by `docs/features/github-issues.md`) at `docs/archive/github-issues-gap-analysis.md`; and the `feature/agent-chat` → `main` merge record at `docs/archive/main-merge-plan.md`
