...
- Left sidebar (two-state expanded ↔ icon rail, with aggregate agent-status dots on collapsed project avatars + per-project hover flyout — Superset-adapted; plus the **living sidebar** since PR #182 / `v0.14.2`: state-driven workspace-row density, a pinned "Needs you" strip, a "Gather on top" LIVE-agents grouping, and a configurable working indicator, replacing the removed `sidebar.workspace_detail` setting): `docs/features/sidebar.md`
