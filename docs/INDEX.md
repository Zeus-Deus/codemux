...
- Context-window meter (composer donut-ring + popover showing live context occupancy `% · used/max`, total-processed, auto-compaction note — `ContextUsageUpdated` event from all three providers, persisted for hydrate-replay; unreleased after `v0.15.5`): `docs/features/agent-chat.md` (§ Context-window meter (composer))
