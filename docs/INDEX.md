...
- Left sidebar (two-state: expanded **flat workspace inbox** ↔ icon rail. The inbox — unreleased, from the `Sidebar Inbox.dc.html` design handoff — replaced the nested project tree (and the PR #182 living-sidebar strip/LIVE-grouping) with search + repo filter chips + one card per active workspace + a persisted "Settled" section, 288px default width, and a Settings → Appearance → Sidebar "Show git stats" toggle; the rail keeps aggregate agent-status dots on project avatars + the per-project hover flyout, and the configurable working indicator remains): `docs/features/sidebar.md`
