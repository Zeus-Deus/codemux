...
| Runtime failure | `addon-host/tests`, native `protocol.rs`, manager fault isolation; packaged five-workload evidence | Windows saved installer passes all five SDK callback workloads; Linux repeat with backpressure fix pending. Native real-child tests cover unexpected exit and hostile shutdown; final-source platform checks remain |
