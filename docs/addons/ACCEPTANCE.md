...
| --- | --- | --- |
| Core independence | Existing full desktop CI; Settings browser evidence; native UI harness pause/resume | Saved installers pass clean/paused zero-host checks, terminal/input/projects, actual themes and read-only official updater checks; final-source repeat and pane restoration remain |
...
| Secrets | `credentials.rs`: real Linux/Windows OS backends, locked-store fallback, source/origin isolation, historical-key cleanup and retry tombstones | Linux installed Settings fallback/removal/redaction passes; Windows OS save/delete passes but harness diagnostic redaction rerun remains |
...
| UI/accessibility | Renderer/view/Settings tests and light/dark/small-window/GUI-off browser screenshots | Both saved installers pass chat-GUI-off panel/terminal behavior, keyboard dialog focus and native light/dark themes; screen-reader inspection, pane restoration and empty accessory/footer spacing remain |
