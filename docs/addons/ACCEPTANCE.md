...
| Persistence | Storage persistence, retained-source checks, corrupt registry and unclean-session tests | Windows saved installer passes actual restart/grants/settings, removal and core startup with corrupt registry; Linux and final-source repeat pending |
...
| UI/accessibility | Renderer/view/Settings tests and light/dark/small-window/GUI-off browser screenshots | Windows saved installer passes actual chat-GUI-off panel/terminal/unavailable-composer behavior; native keyboard/screen-reader checks, pane restoration and empty accessory/footer spacing remain |
