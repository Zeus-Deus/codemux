...
- Status: ACTIVE — Stages A–C code landed on `main`, and the gated hosted deploy has happened: `get.codemux.org/install.sh` serves the real installer, `api.codemux.org/health` returns `{"status":"ok"}` with `/api/devices` returning `401` unauthenticated, and `app.codemux.org` serves the hosted client (probed 2026-08-08). An end-to-end from-anywhere round trip against the deployed service has not been re-verified here. 8 design questions remain unresolved.
