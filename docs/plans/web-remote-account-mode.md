...
- Authority: Active design/plan. **Design 1 Stage A has shipped** — its current truth now lives in `docs/features/web-remote-access.md` § "Account-mode admission"; everything else here (Stages B/C, headless server mode) is still design-only. The locked v1 contract is in `docs/plans/web-remote-access.md`.
...
| **B — Control-plane device registry + discovery** | `codemux_devices` table + register/list/connect endpoints on the VPS; desktop self-registers `NodeId`/device pubkey when account mode on; browser device-picker. Transport still LAN/Tailscale, but the browser now *finds* the device by account. Introduce `Identity::Cloud` on the web-remote admission path. | **VPS deploy (gated, confirm-first).** New table + endpoints + tests, `docker compose up -d --build`, redeploy `preload.ts` parity. | High — a near-copy of the `codemux_workspaces` endpoints; well-trodden. | Med | Med (schema/deploy discipline; VPS is production). | Stage A (LAN account pairing). |
| **C — E2E connectivity substrate (iroh)** | Desktop iroh endpoint + `NodeId` registration; browser WASM iroh transport in `src/remote/transport.ts` behind the same frame contract; our iroh relay running on the VPS (new docker-compose service, QUIC/UDP + HTTPS, Traefik/UDP wiring); connect-grant → iroh dial. Now truly from-anywhere, no user network setup, no plaintext at the relay. | **VPS deploy of the relay + control-plane connect endpoint (gated, confirm-first).** | Medium — iroh is production-grade but its browser/WASM transport and a self-hosted relay are the least-proven pieces here; needs a spike. | High | Med–High (new crypto/transport surface; relay ops; WASM bundle size in the shim). | Stage B endpoint (LAN/Tailscale) if iroh direct+relay fail; pairing token if the control plane is down. |
