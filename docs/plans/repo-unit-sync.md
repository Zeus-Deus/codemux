...
- **Project-first pull with a real protected root.** Root rows recorded a null branch, so the desktop fell back to literal `"main"` and `is_protected_repo_root` fail-opened to "deletable"; local-only repos had no default-branch signal at all. Fixed by promoting `find_default_branch` to `pub`, adding `resolve_default_branch` + `ensure_origin_head`, threading a **local-only `default_branch`** column end-to-end (daemon → poller → `workspaces_sync` → TS; not sent to cloud), and adding `workspaces_adopt_project` which materialises the protected root first then recreates each worktree under it. UI: sidebar delete respects `protected`; the overview floats the root to the top of its cluster and adds **"Pull project"**.
- **One-click "Reconcile copy" (the Phase 5 follow-up).** `workspaces_reconcile_copy` non-destructively detaches a divergent standalone-copy card (files always left on disk), refusing while it has uncommitted/unpushed work (`git::reconcile_copy_blocker`). The user then runs "Pull project" for a clean protected root. The auto-rewrite variants (re-point a clean copy / import a dirty copy as a branch) remain intentionally out — `reconcile_copy` + `adopt_project` cover the workflow without rewriting git state.
- **Collision-safe host paths.** Pushed/pulled/spawned host paths are now keyed on `project_uid` (`conventional_remote_*_keyed` / `project_dir_component` → `<basename>-<short-uid>`) so two different repos sharing a basename no longer overwrite each other; the legacy basename layout is reproduced exactly when no uid is known (migration safety net + pull-back fallback).
- **Collision-safe LOCAL adoption landings + JSONL sync (issue #65).** The re-key is now complete on the local side: adoption landing paths are uid-keyed (`adopt_root_landing`/`adopt_worktree_landing` in `commands/workspaces_sync.rs`, shared by the preview + host-backed + clone + worktree-rsync paths, with a `choose_landing` basename read-fallback so already-adopted copies still resolve and re-pull stays idempotent), and the Claude `.jsonl` session-continuity sync derives the remote dir from the workspace's actual on-host path (`resolve_remote_cwd` in `commands/hosts.rs`) instead of recomputing the basename.
- **One repo root per project (daemon-side).** `collapse_main_for_uid` + a boot-time `normalize_main_workspaces` sweep collapse duplicate `kind=main` rows for one `project_uid` (registry rows only; files untouched), and `WorkspaceStore::create` collapses after inserting a `main` row so re-registration is idempotent.

## Not Yet Done / Validation Gaps

- **SSH round-trip not exercised end-to-end.** The host landing path for repo roots (now uid-keyed) is unit-tested but the actual cross-device rsync round-trip could not be run in the dev sandbox (no second machine). Validate on real hardware: push a repo root to a host → confirm it lands at `~/.codemux/projects/<repo>-<uid>` (not the worktrees tree); adopt it on a third device → confirm a protected root, not a divergent copy; confirm a legacy root pushed before this change still pulls (basename fallback). **Also covers issue #65's local-side re-key:** adopt two *different* same-basename sibling projects onto one device → confirm they land in distinct `<basename>-<uid>` dirs (not one clobbering the other); confirm a workspace adopted *before* the re-key (bare-basename dir on disk) re-pulls into the same dir instead of duplicating; and confirm Claude conversation history round-trips for a uid-keyed pushed workspace (push with local `.jsonl` history → `claude --resume` on the host finds it; continue there → pull back → continuation is present locally).
- The three flags (`protected` / `divergent_copy`) + `default_branch` were NOT added to the cloud schema (kept local-only). Promote only if cross-device protection/labels prove necessary.
