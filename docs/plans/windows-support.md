...
- [x] **`.github/workflows/release.yml:10` — `runs-on: ubuntu-22.04` only; installs Linux-only deps (`libwebkit2gtk-4.1-dev`, `libgtk-3-dev`, `librsvg2-dev`, `libfuse2`)**
  ...
  - **Release pipeline (2026-04-12)**: `release.yml` now also builds on a `[ubuntu-22.04, windows-latest]` matrix with `fail-fast: false`. Ubuntu is pinned at 22.04 (not ubuntu-latest) so the AppImage stays glibc-compatible with older distros when GitHub rolls ubuntu-latest to 24.04. Both platforms share the same Setup Rust / Rust cache / Setup Node / npm ci / agent-browser pre-stage / git identity steps. The `tauri-apps/tauri-action@action-v0.6.2` call is split into two conditional invocations — one for `ubuntu-22.04` and one for `windows-latest` — because the Linux side passes `TAURI_SIGNING_PRIVATE_KEY` env vars and `includeUpdaterJson: true`, while the Windows side omits both and adds `args: --bundles nsis`. **Why the split**: (a) `latest.json` can only have one canonical source — if both jobs tried to upload it, the second would overwrite the first's platform entries. Linux stays canonical; Windows users get a downloadable `.exe` in release assets but no auto-update for v1. (b) `bundle.targets = "all"` in tauri.conf.json would otherwise build both NSIS and MSI on Windows, and MSI requires the WiX Toolset which is NOT preinstalled on `windows-latest` runners — the build would fail with "wix not found". Restricting to `--bundles nsis` sidesteps the problem. **What's still explicitly not done**: Windows Authenticode code signing (SmartScreen warning on unsigned builds — budget decision, TODO comment added inline in the workflow), macOS matrix entry (separate follow-up).
