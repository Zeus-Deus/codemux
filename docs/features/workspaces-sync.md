...
Local-only sync columns (threaded device→device through the host-inventory poller but **never sent to the cloud API**, so they add no server schema): `origin_uid`, `origin_path` (see "Identity contract for remote-discovered rows"), and `default_branch` (`v0.7.9`). The daemon stamps `default_branch` at creation and backfills legacy rows on boot; the host poller threads it into the `workspaces_sync` row/view/TS so a project-first pull can checkout the repo's real default branch and classify protection against it — see "Project-first pull + protected root" below.
...
- **Cross-device race for the same host workspace.** `origin_uid` is local-only. If Device A and Device B both poll Pandora between cloud pulls, both publish a row for the same physical workspace and the overview briefly shows two entries. **Fixed server-side (issue #66):** `POST /api/workspaces` now upserts (`ON CONFLICT`) against a partial unique index on the workspace's stable identity — `(user_id, host_server_id, project_uid, git_branch, workspace_kind) WHERE deleted_at IS NULL AND project_uid IS NOT NULL AND host_server_id IS NOT NULL`, `NULLS NOT DISTINCT` (in the `codemux-api` repo) — so the racing second insert updates the existing row and both devices converge on one cloud row with the same id, no client pass required. The server can only key on the `project_uid` fallback because `origin_uid` is never synced. `dedupe_sibling_rows` (see "Client-side duplicate collapse on pull") is **retained as defense-in-depth** for rows created before the index shipped or by an older server. Single-device users — the common case today — never hit this anyway.
...
`dedupe_sibling_rows` (run at the tail of `reconcile_from_snapshot`) converges the cross-device race where two devices poll the same host before either's push lands and the cloud ends up with two rows for one physical workspace. It groups **un-adopted** siblings (`workspace_id IS NULL`) by `(host, origin_uid)` — or `(host, project_uid, branch, kind)` since `origin_uid` is local-only and absent on cloud-pulled rows — keeps the canonical row (one with a `server_id`, lowest id) and tombstones the rest so the next push removes the cloud duplicate. **Adopted rows are never touched.** As of **issue #66 the permanent fix has shipped server-side**: `POST /api/workspaces` upserts against a partial unique index on `(user_id, host_server_id, project_uid, git_branch, workspace_kind)` (the same `project_uid` fallback identity this pass uses; `origin_uid` is never synced), so the duplicate can no longer be created. This client-side pass is now **defense-in-depth** — it cleans up duplicates created before the index shipped or by an older server, and converges on the same keeper (lowest id) as the server's startup backfill.
...
- `src-tauri/src/commands/workspaces_sync.rs` — Tauri command surface: `workspaces_sync_list`, `workspaces_sync_now`, `workspaces_adoption_preview`, `workspaces_adopt_synced`, `workspaces_adopt_via_clone`, `workspaces_adopt_project` (project-first pull, `v0.7.9`), `workspaces_reconcile_copy` (`v0.7.9`). Adopt serialization: `adopt_locks()` + `acquire_adopt_lock(server_id)` per-row async lock. Uid-keyed local adoption landings (issue #65): `adopt_root_landing` / `adopt_worktree_landing` / `adopt_worktree_landing_via_repo` + the `choose_landing` basename read-fallback, shared by the preview + all three adopt paths so they agree and re-pull stays idempotent.
- `src-tauri/src/workspaces_sync.rs::dedupe_sibling_rows` — client-side cross-device duplicate collapse (`v0.7.9`), run at the tail of `reconcile_from_snapshot`.
- `src-tauri/src/remote/workspace.rs` — `collapse_main_for_uid` / `normalize_main_workspaces` daemon-side one-root-per-project (`v0.7.9`); `WorkspaceStore::create` collapses after inserting a `main` row.
- `src-tauri/src/workspace_paths.rs` — `project_dir_component` + `conventional_remote_path_keyed` / `conventional_remote_root_path_keyed` uid-keyed host paths (`v0.7.9`, collision-safe).
- `src-tauri/src/git.rs` — `resolve_default_branch`, `ensure_origin_head`, `reconcile_copy_blocker` (`v0.7.9`); `find_default_branch` promoted to `pub`.
