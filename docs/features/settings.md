...
- **Usage** (Agent Chat GUI only): token and cost accounting per provider, model and session over today / 7 days / 30 days, read from the local `agent_usage_ledger`. Separates *metered* spend (OpenCode, on the user’s own API keys) from *plan-covered* list-price value (Claude, Codex) rather than summing them; subagent tokens are counted and flagged, not dropped. Includes a CSV export. Billing classification is hardcoded in v1 (see `docs/features/usage-dashboard.md`)
