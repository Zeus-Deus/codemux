...
- **Persona-based classification (April 2026).** Every terminal session carries a `persona: Persona` field (`Human` or `Agent`, see `src-tauri/src/presets.rs`). `spawn_pty_for_session` reads it and picks a policy via `ExecutionPolicy::worktree_session_default_for_persona(persona)`. The persona is used for preset classification, UI affordances, and future per-persona policy knobs.
- **GUI env isolation is now opt-in (April 2026, superseded the earlier persona-based default-strip approach).** Both `Persona::Human` and `Persona::Agent` default to inheriting the host `DISPLAY`/`WAYLAND_DISPLAY`/`HYPRLAND_INSTANCE_SIGNATURE`/etc. Rationale:
    - **Env-strip was structurally leaky.** libwayland auto-discovers `wayland-0` via `XDG_RUNTIME_DIR` (which we don't strip); libdbus falls back to `$XDG_RUNTIME_DIR/bus` even when `DBUS_SESSION_BUS_ADDRESS` is neutralized; Hyprland's IPC socket is enumerable under `$XDG_RUNTIME_DIR/hypr/`. So the strip never actually prevented the GUI pop-ups it was advertised to block.
    - **It broke legitimate uses.** Claude Code's `Ctrl+V` image paste shells out to `xclip` → `wl-paste`, both of which require `DISPLAY`/`WAYLAND_DISPLAY`. Every other ADE (Cursor, Codex CLI, Conductor, T3Code, Claude Code itself) runs agents in the user's session with full env; matching them removes the regression without giving up a protection that wasn't actually holding.
  Isolation remains available as an **opt-in**:
    - `CODEMUX_ALLOW_DESKTOP_GUI=0` (force-deny globally) — re-engages the full strip list + neutralizers for every session.
    - `.codemux/config.json` → `{"sandbox": {"allow_desktop_gui": false}}` — per-workspace force-deny.
    - `CODEMUX_VIRTUAL_DISPLAY=1` combined with force-deny routes agents to a headless per-workspace Xvfb (Phase 2).
  The `sanitize_gui_env_std/tokio` helpers, `gui_env_keys()`, `gui_env_overrides()`, and `build_linux_bwrap_args` isolation paths are all preserved — they engage when the user opts in.
