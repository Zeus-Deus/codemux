...
- Current truth lives here. The from-anywhere account/relay/hosted tier (iroh transport, device registry, hosted `app.codemux.org` client, in-browser GitHub OAuth) has **landed on `main` but its hosted service is not yet deployed** — its design + stages live in `docs/plans/web-remote-account-mode.md` and the deploy runbook in `docs/plans/app-codemux-org-hosting.md`. Remaining next steps are per-client views, per-command ACL scopes, browser-pane screencast confirmation on a real display, updater UX against a published release, hosted-service deployment, and broader platform validation of `codemux serve`.
