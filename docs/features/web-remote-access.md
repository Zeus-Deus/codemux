...
- `mod.rs` — managed `WebRemoteState`, config persistence, server lifecycle, the Tauri command surface, the boot-restore + e2e-autostart hooks, and the same-machine `control_enable`/`control_disable`/`control_pair` helpers the CLI drives.
- `serve.rs` — the headless `codemux serve` entrypoint (`ServeOptions`, `run_serve`): boots the full backend headless, enables the server through the shared `control_enable` path, prints the pairing QR + link, and blocks on SIGINT/SIGTERM.
...
On the browser, the pre-app bootstrap (`src/remote/{bootstrap.tsx,account-pair.ts}`) offers "Sign in with your Codemux account" (email + password) when the server advertises account mode, alongside the paste-a-code path, and handles the approval-pending state with the same "waiting for approval" screen the pairing path uses. On the LAN/tailnet transport, account-mode admission is email/password only; **GitHub-OAuth-in-the-browser is available on the hosted from-anywhere client** (`app.codemux.org`, below), where the browser can complete a real OAuth round-trip against `api.codemux.org`.

### From-anywhere tier: iroh relay + hosted client (landed on `main`, hosted service not yet deployed)

The LAN/tailnet base above requires the browser and desktop to already share a network. The **from-anywhere tier** (Stages B/C of `docs/plans/web-remote-account-mode.md`) removes that requirement — code has landed on `main` (all default-off), though the hosted service is not yet stood up. It is a pure **transport swap under the same `/ws` frame contract**; the invoke/event/channel protocol, auth session model, and fan-out are unchanged.

- **The desktop as an iroh endpoint.** Gated by a **relay-mode** toggle (Settings → Remote Access), the desktop becomes an [iroh](https://github.com/n0-computer/iroh) (`iroh = "1"`) QUIC endpoint (`src-tauri/src/web_remote/iroh.rs`), self-registers its `NodeId` + device name with the `api.codemux.org` device registry (`registration.rs` → `POST /api/devices`), and accepts inbound iroh bi-streams. The existing `/ws` frames ride *inside* the iroh stream verbatim.
- **The hosted static client.** A browser on any network loads `app.codemux.org` — a **static bundle** (built via `npm run build:iroh-wasm` to stage the browser iroh WASM client, then `VITE_CODEMUX_HOSTED=true npm run build`; hosted branch keyed by `isHostedOrigin()` in `src/remote/hosted.ts`). It signs into the Codemux account (**GitHub OAuth** via `src/remote/hosted-oauth.ts` — `/api/auth/web/connect` → browser-return `/api/auth/web/exchange` — or email/password), lists the account's devices (`src/remote/device-registry.ts`), picks one, mints a signed **connect-grant** (`POST /api/devices/:id/connect`), and dials the desktop's `NodeId` over iroh (`src/remote/iroh-transport.ts`, `iroh-connection.ts`, `iroh-wasm-loader.ts`, `iroh-codec.ts`).
- **Data path stays E2E, decision-9 clean.** Browser ⇄ desktop traffic is E2E-encrypted QUIC — hole-punched directly when possible, otherwise forwarded as **ciphertext** through public iroh relays. Neither a relay nor the `app.codemux.org` static host ever sees plaintext PTY/agent/source bytes. The control plane (`api.codemux.org`) does discovery + grant-brokering only, never carrying session bytes. The desktop verifies each inbound grant is for its own account + `NodeId` before admitting a session.
- **Not yet live.** The `app.codemux.org` static host and the `api.codemux.org` device-registry + grant-verify endpoints are **gated human deploys** — nothing in this repo deploys them (runbook: `docs/plans/app-codemux-org-hosting.md`). Real cross-network hole-punching / relay-fallback connectivity and throughput are therefore exercised only against mocked verify endpoints + unit/integration tests until the first live deploy.

### Controlling remote access from the terminal — `codemux remote enable/disable/pair`

When you're away from home you can SSH into the desktop and drive the whole remote-access lifecycle from the shell, without opening the GUI. The control socket is same-machine + unauthenticated by design — over SSH you **are** on the machine — so it is the right transport. Each command sends a control request (`control.rs::dispatch_request`) that calls the **same** `web_remote` code the Tauri commands use, so there is no second implementation of the config/bind/rollback logic.

`codemux remote enable [--scope all|tailscale|loopback] [--port N]` (`web_remote_enable` control command → `web_remote::control_enable`) turns the server on:

- Off → on: it folds the requested scope/port into the persisted config, then runs the **same** `enable_core` path the `web_remote_enable` Tauri command uses (bind, and roll `enabled` — plus the scope/port — back on a bind failure, so `--scope tailscale` on a host with no tailnet address fails with the clear `No Tailscale address found …` error and leaves the server off on its last-good config).
- Already on with a `--scope`/`--port` flag: it is treated as a config change and rebinds through the **same** `web_remote_set_config` path the Settings pane uses (dropping live connections, restoring the last-good address if the new one can't bind).
- Already on with no flags: it reports that it's already running with the current status.

On success it prints the resulting port, bind scope, and the single **recommended** reachable endpoint (the same MagicDNS → tailnet → local-network → loopback pick the Settings pane and `codemux remote pair` surface) so you can immediately run `codemux remote pair`. `codemux remote disable` (`web_remote_disable` → `web_remote::control_disable`) turns it off through the shared `disable_core` path — severing every live socket, tearing the iroh endpoint down, and stopping registration. Both require the desktop app to be running; when it isn't, the existing `Failed to connect to Codemux control endpoint` error prints.

`codemux remote pair [--name <label>]` sends a `web_remote_pair` control command (`control.rs::dispatch_request`), which calls `web_remote::control_pair`:
...
### Headless server mode — `codemux serve`

`codemux serve [--scope all|tailscale|loopback] [--port N] [--relay]` runs Codemux as a **web-remote server with no desktop GUI** — the away-from-home SSH flow where there is no display to open a window on. It boots the *full* backend headless (`web_remote::serve::run_serve` → `codemux_lib::build_headless_app()` on `tauri::test::MockRuntime` — DB, state, control server, PTY warmup, `restore_on_boot`, every background loop, and the in-memory `"main"` webview the invoke dispatch drives), so the browser gets the identical command surface the desktop does. The dispatch path (`web_remote/dispatch.rs` → `WebviewWindow::on_message`) works headless without the desktop window ever being present or visible.

- **Flags mirror `codemux remote enable` exactly.** `--scope` takes the same three values; `--relay` additionally flips `relay_mode_enabled` and starts the iroh endpoint (through the same `web_remote_set_config` path). Binding, bind-scope validation, rollback, config persistence, and `web-remote-state-changed` all go through the shared `control_enable` path — behavior is identical to the GUI/CLI. Scope resolution: an explicit `--scope` wins; with no flag, a config that was already persisted-enabled keeps its scope (`restore_on_boot` may have already bound it — the `already_running` case is handled), otherwise the first-run default is `all` (the SSH use case).
- **Mutual exclusion with the GUI is symmetric.** `serve` refuses to start (clear one-line error, non-zero exit) when `control::control_server_is_running()` reports another instance (GUI or `serve`) already holds this machine's control endpoint; the GUI's boot path (`main.rs`) reciprocally refuses when a `serve` instance does. `serve` itself spawns the control server, so `codemux remote pair` from another SSH session mints fresh codes against the running instance, and account-mode / hosted access keeps working if configured.
- **Startup banner + pairing.** On boot it prints the bound port, bind scope, and the reachable endpoints (the shared enumeration), then mints a pairing token via `control_pair` and prints it with the same `print_pairing` / `render_qr` the `codemux remote pair` CLI uses (QR + link + token + expiry) — no duplicated formatting. If token minting fails it logs the error but keeps serving.
- **Lifecycle.** `serve` is a long-lived foreground process, so `main` dispatches it on the **main thread** (via `CliOutcome::RunServe` from `maybe_run_cli`, not inside `block_on(maybe_run_cli())`) rather than as a control round-trip. It holds the app and blocks on SIGINT + SIGTERM (`tokio::signal`); no `app.run()` is called (MockRuntime has no event loop). On signal it performs the same `RunEvent::Exit` cleanup the GUI does (`agent_browser::kill_stream_daemons()`) and exits 0.
- **Debug-asset caveat.** In debug builds the embedded frontend is empty (the desktop dev build serves from Vite's `devUrl`), and the web-remote server falls back to a source-tree `dist/`. When neither is present, `serve` prints a one-line warning that the web UI bundle is unavailable (pairing still works). Release builds embed the bundle — no warning.
...
- **No plaintext PTY bytes through any cloud service.** Browser ⇄ desktop traffic is direct on the LAN/tailnet transport. The from-anywhere iroh tier (above) is decision-9 clean too: its data path is E2E-encrypted QUIC, so a relay-of-last-resort forwards only ciphertext and the control plane never carries session bytes.
- **Account-mode admission is opt-in and desktop-verified.** With account mode on (default off), a browser can obtain a session by proving it owns the desktop's own Codemux account (`POST /api/pair-account`, above) — but the raw password never leaves the browser (only the derived AuthSecret does, to the user's own desktop), the desktop verifies the resolved `user.id` equals its own signed-in user, and account sessions default to pending approval. On the LAN/tailnet transport this is email/password only; the hosted from-anywhere client (`app.codemux.org`) adds a real in-browser **GitHub OAuth** round-trip. Pairing stays the offline/LAN path and inherits the desktop's account for everything downstream.
...
- `src-tauri/src/control.rs` — the `web_remote_enable` / `web_remote_disable` / `web_remote_pair` control-socket commands (same-machine, drive `web_remote::control_enable` / `control_disable` / `control_pair`); `src-tauri/src/cli.rs` — the `codemux remote enable [--scope …] [--port N]` / `remote disable` / `remote pair [--name <label>]` subcommands + `print_enable_result` / `print_pairing` formatters + `render_qr` terminal QR (`qrcode` dep in `Cargo.toml`), plus the `codemux serve` subcommand and the `CliOutcome` signal (`Handled` / `LaunchGui` / `RunServe`) `maybe_run_cli` returns to `main`.
- `src-tauri/src/web_remote/serve.rs` — the headless `codemux serve` entrypoint: `ServeOptions` + `run_serve` (mutual-exclusion check, `build_headless_app`, shared `control_enable` bind, `--relay` via `web_remote_set_config`, banner + `control_pair`, SIGINT/SIGTERM keep-alive, `RunEvent::Exit`-parity teardown).
- `src-tauri/src/main.rs` — dispatches `CliOutcome::RunServe` on the main thread (long-lived, not a control round-trip); the OpenFlow-agent GUI guard applies to `LaunchGui` only, so an explicit `serve` is allowed like any other subcommand.
- `src-tauri/src/lib.rs` — registers `WebRemoteState`, wires the channel interceptor to `channel_router().route`, calls `restore_on_boot` + `e2e_autostart` in `setup`, lists the `web_remote_*` commands in `generate_handler!`, and exposes `build_headless_app()` + `run_serve()` for the serve entrypoint.
...
- Current-truth lives here. The from-anywhere account/relay/hosted tier (iroh transport, device registry, hosted `app.codemux.org` client, in-browser GitHub OAuth) has **landed on `main` but its hosted service is not yet deployed** — its design + stages live in `docs/plans/web-remote-account-mode.md` and the deploy runbook in `docs/plans/app-codemux-org-hosting.md`; fold each piece's behavior into this doc as it goes live. Remaining next steps (per-client views, per-command ACL scopes, browser-pane screencast confirmation on a real display, updater UX against a published release, headless server mode) live in `docs/plans/web-remote-access.md` and `docs/plans/web-remote-account-mode.md`.
