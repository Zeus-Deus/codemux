...
**Migration overlay (issue #23).** The stop-sync-restart step terminates each pane's local PTY and respawns it on the remote daemon, a few seconds during which the user would otherwise stare at frozen local scrollback. Between `terminate_workspace_sessions` and `spawn_missing_ptys_for_workspace`, both `workspace_push_to_host` and `workspace_pull_back_impl` call `crate::terminal::emit_migrating_for_workspace`, which emits a transient `Migrating` lifecycle status (`terminal/mod.rs`) for every session in the workspace — message `"Switching to <host>…"` on push, `"Returning to this device…"` on pull-back. The frontend overlay (`TerminalPane.tsx`) renders it as a distinct "Migrating workspace" card and auto-dismisses (short opacity fade) the moment the respawned session goes `Ready` or its first output byte reaches the renderer. `Migrating` collapses to `Starting` in the persisted session state, so only the `terminal-status` event carries the discriminant — no new persisted/overview variant. See `docs/features/terminal.md` for the overlay rendering details.
