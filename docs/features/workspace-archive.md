...
- **Archive Project** (**currently unreachable** — its only entry point is the project-header context menu in `sidebar-project-group.tsx`, which PR #198's workspace inbox unmounted; per-workspace archive is unaffected and still works from the inbox card). The action archives every member (worktrees first, root last, sequentially so teardown scripts don't race); attach-in-place/remote members get a plain close. Chat drafts for the project are cleared only when every member archived cleanly.
...
- `src/components/layout/sidebar-project-group.tsx` — Archive Project flow (**unmounted**: zero non-test importers since PR #198)
