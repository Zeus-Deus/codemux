...
- **Unarchive.** `unarchive_workspace` restores through the same creation paths used elsewhere: worktree entries go through the worktree-creation flow (which reuses the still-on-disk directory for the branch, or checks the surviving branch out fresh if the directory is gone), root entries through the add-repository flow (re-stamping `protected`/`project_uid`). The archive entry is only removed after a successful restore; the restored workspace keeps its recorded title and is activated. If a live workspace already sits at the entry's location, unarchive just switches to it. If both the worktree directory and its branch are gone, unarchive errors and keeps the entry.
