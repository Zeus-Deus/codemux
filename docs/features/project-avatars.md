...
- **State lives in `src/stores/project-appearance-store.ts`**, shared by the one writer (the context-menu project section) and every reader. A save therefore repaints all surfaces of that project *immediately* — the previously read-only `useProjectAppearance` hook cached per component, so a write only showed up on the next remount. The store also dedupes the read, so a project costs one DB round-trip no matter how many of its avatars are mounted.
- **Write-wins merge on a racing load.** The store keeps a per-project `writtenFields` dirty set, and `load` merges the persisted read **field by field**: a field the user has written keeps the in-store value (including an explicit `null`, i.e. a deliberate clear), while untouched fields still take their persisted values. The dirty-set matters because the naive guard — discarding the whole read when anything was written — would drop the persisted *image* that same read returned just because a color pick landed first, and vice versa. Both race directions are pinned by tests.
...
- A save repaints every appearance-store surface of that project at once (all its cards, settled and snoozed rows, the collapsed rail, the project filter dropdown). **One surface is outside the store**: the needs-you strip reads `project.color:<path>` directly via `dbGetUiState`, once per change of the blocked-project set, and renders **color only — never a custom image**. So a save reaches the strip late (on its next fetch), and an image set for a project never appears there.
