...
export function QuestionForm({
  ...
  const [picks, setPicks] = useState<Record<number, Set<string>>>(() => {
    ...
    questions.forEach((_, i) => {
      init[i] = new Set(
        questions[i].options.some((o) => o.label === initialAnswers[i])
          ? [initialAnswers[i]]
          : [],
      );
    });
  ...
  const [otherText, setOtherText] = useState<Record<number, string>>(() =>
    Object.fromEntries(
      initialAnswers.map((a, i) => [
        i,
        questions[i]?.options.some((o) => o.label === a) ? "" : a,
      ]),
    ),
  );
  ...
  const collectAnswers = useCallback((): string[] => {
    // Preserve question order; duplicate question titles are valid.
  ...
  }, [collectAnswers, onAnswersChange]);
  ...
  }, [enabled, globalShortcuts, questions, qi]);
              ...
              >
                ...
                {q.allowOther && (
                  <OtherRow
                    ...
                    idPrefix={idPrefix}
                    placeholder={
                      q.options.length ? "Something else…" : "Your answer…"
                    }
                    value={otherText[i] ?? ""}
                    onChange={(val) =>
...
function OtherRow({
  ...
  return (
    <div className="relative flex w-full items-center">
      <QuestionnaireInput
        id={`${idPrefix}-${questionIndex}-other`}
