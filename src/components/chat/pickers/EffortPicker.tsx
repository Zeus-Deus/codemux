import { useState } from "react";
import { Brain, Check, ChevronDown } from "lucide-react";

import {
  Command,
  CommandEmpty,
  CommandGroup,
  CommandItem,
  CommandList,
} from "@/components/ui/command";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import { cn } from "@/lib/utils";
import type { ChatModelInfo } from "@/tauri/types";
import { focusCmdkRootOnOpen } from "./focus-cmdk-root";

// Short description lines for each effort level. Verbs match
// PermissionModePicker's density (two-line rows). Keep copy short —
// these appear right below the title in muted text.
const DEFAULT_EFFORT_DESCRIPTIONS: Record<string, string> = {
  none: "No extra reasoning",
  minimal: "Fastest, minimal reasoning",
  low: "Light reasoning",
  medium: "Balanced default",
  high: "Thorough reasoning",
  xhigh: "Extra-thorough reasoning",
  max: "Deepest reasoning",
  ultrathink:
    "Prepends \"Ultrathink:\" to your prompt for extra-thorough reasoning",
};

interface Props {
  /** Active chat model, or null when capabilities haven't loaded. */
  model: ChatModelInfo | null;
  /** Current selected effort from the thread slice, or null for default. */
  value: string | null;
  /** Canonical label map from the provider capabilities — wins over
   *  locally-generated ids when set. */
  labelMap: Record<string, string>;
  /** True when the composer's draft text contains "ultrathink" outside
   *  the canonical prefix — disables the picker. */
  ultrathinkInBodyText: boolean;
  /** Fired on every selection. `"ultrathink"` carries the special
   *  meaning of "prepend to the prompt"; caller handles that. */
  onChange: (nextValue: string) => void;
  disabled?: boolean;
}

function effortLabel(labelMap: Record<string, string>, id: string): string {
  return labelMap[id] ?? id;
}

function effortDescription(id: string): string {
  return DEFAULT_EFFORT_DESCRIPTIONS[id] ?? "";
}

export function EffortPicker({
  model,
  value,
  labelMap,
  ultrathinkInBodyText,
  onChange,
  disabled,
}: Props) {
  const [open, setOpen] = useState(false);

  if (!model) return null;

  // Combined native + prompt-injected levels. Both surface in the
  // menu; the caller distinguishes them on selection.
  const levels = [
    ...model.effort_levels,
    ...model.prompt_injected_effort_levels,
  ];

  // Haiku and other models without effort levels: hide entirely.
  if (levels.length === 0) return null;

  const current =
    value && levels.includes(value) ? value : model.default_effort ?? levels[0];
  const triggerLabel = effortLabel(labelMap, current);

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          disabled={disabled}
          className="inline-flex items-center gap-1.5 rounded-full bg-muted/60 px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground outline-none disabled:opacity-50"
        >
          <Brain className="h-3 w-3" />
          <span className="max-w-[140px] truncate">{triggerLabel}</span>
          <ChevronDown className="h-2.5 w-2.5 opacity-40" />
        </button>
      </PopoverTrigger>
      <PopoverContent
        className="w-[340px] p-0"
        align="start"
        onOpenAutoFocus={focusCmdkRootOnOpen}
      >
        <Command>
          {ultrathinkInBodyText ? (
            <div className="px-3 pt-2 pb-1 text-[11px] text-muted-foreground/80">
              Your prompt contains &quot;ultrathink&quot; in the text. Remove
              it to change effort.
            </div>
          ) : null}
          <CommandList className="max-h-[340px]">
            <CommandEmpty>No effort levels</CommandEmpty>
            <CommandGroup>
              {levels.map((level) => {
                const isDefault = level === model.default_effort;
                const title = effortLabel(labelMap, level);
                const description = effortDescription(level);
                return (
                  <CommandItem
                    key={level}
                    value={level}
                    disabled={ultrathinkInBodyText}
                    onSelect={() => {
                      if (ultrathinkInBodyText) return;
                      onChange(level);
                      setOpen(false);
                    }}
                    className="h-auto gap-2 py-2"
                  >
                    <div className="flex flex-1 flex-col min-w-0">
                      <span className="text-xs text-foreground truncate">
                        {title}
                        {isDefault ? (
                          <span className="ml-1.5 text-muted-foreground/60">
                            (default)
                          </span>
                        ) : null}
                      </span>
                      {description ? (
                        <span className="text-[11px] text-muted-foreground/80 truncate">
                          {description}
                        </span>
                      ) : null}
                    </div>
                    <Check
                      className={cn(
                        "h-3.5 w-3.5 text-muted-foreground",
                        current === level ? "opacity-100" : "opacity-0",
                      )}
                    />
                  </CommandItem>
                );
              })}
            </CommandGroup>
          </CommandList>
        </Command>
      </PopoverContent>
    </Popover>
  );
}
