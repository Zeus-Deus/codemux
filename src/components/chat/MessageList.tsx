    ...
    <ChatFileLinkContext.Provider value={fileLinkContext}>
      ...
      {showJumpToLatest && (
        <Button
          ...
          className="absolute left-1/2 z-10 h-8 w-auto -translate-x-1/2 gap-1.5 rounded-full border border-border bg-card px-3.5 text-body-sm font-semibold text-muted-foreground shadow-lg hover:bg-card hover:text-foreground"
        >
          Jump to latest
...
function RunInterruptedDivider() {
  return (
    ...
    >
      ...
      <span className="font-mono text-label font-medium tracking-wide">
        Run interrupted
      </span>
      <span className="h-px flex-1 bg-warning/40" />
    </div>
  ...
  switch (item.kind) {
    ...
    case "runtime_notice":
      ...
      return item.severity === "error" ? (
        <div className="select-text border-l-2 border-destructive/40 bg-destructive/10 px-3 py-1.5 text-body-sm text-destructive">
          {item.message}
        </div>
      ) : (
        <div className="select-text border-l-2 border-status-working/40 bg-status-working/10 px-3 py-1.5 text-body-sm text-status-working">
          {item.message}
        </div>
      );
    case "subagent_run":
