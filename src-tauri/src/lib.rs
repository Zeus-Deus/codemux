...
/// Include the embedded-asset context exactly once. The context (which
/// embeds the entire frontend bundle into the binary) is generated by
/// build.rs via `tauri_build::CodegenContext`; including it in a single
/// generic function (rather than at each `.build()` site) keeps the assets
/// from being embedded twice. It expands to a generic `Context<R>`, so both
/// the Wry GUI app and the MockRuntime headless app share this one copy.
...
fn build_core_app<R: tauri::Runtime>(
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::addons::addon_registry_reset,
            commands::addons::addon_catalog,
            commands::addons::addon_catalog_review,
            commands::addons::addon_check_update,
            ...
            commands::set_pr_ready,
            commands::update_pull_request,
            commands::request_pr_review,
            commands::get_check_log_excerpt,
            commands::get_pull_request_checks,
            commands::get_pr_review_comments,
            commands::get_pr_inline_comments,
            commands::get_pr_review_threads,
