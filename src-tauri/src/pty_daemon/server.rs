...
async fn spawn_pty(
    ...
    // vars or the pinned sidecar path, which are an app-process concern and
    // must never reach a shell or anything launched from one.
    crate::terminal::strip_app_process_env(&mut cmd);
